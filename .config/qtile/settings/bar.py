...
def create_bar(has_systray: bool = False) -> Bar:
    ...
    widgets = [
        ...
        widget.WindowName(**powerline_config),
        widget.Spacer(**powerline_config),
        widget.Clock(
            format="󰥔 %B %d %H:%M",
        ...
        widget.Battery(
            ...
            mouse_callbacks={"Button1": lazy.spawn(f"{pterm} --class btop -e btop")},
            # **powerline_config,
        ),
    ]

    if has_systray:
        widgets.append(widget.Systray(background=colors["orange"], icon_size=20))

    return Bar(widgets, 26, background=colors["background"], margin=[8, 8, 0, 8])
...
screens: list[Screen] = [Screen(top=create_bar(m == 0)) for m in range(num_monitors)]
